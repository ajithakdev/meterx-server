...
5.  **Important:** If you self-host the server, you'll need to update the `SERVER_URL` variable in the extension's `popup.ts` file to point to your server's address.
...
│   ├── popup.ts          # Popup TypeScript code
│   ├── background.ts     # Background service worker TypeScript code
...
│   ├── vite.config.ts    # Vite configuration for server
│   └── package.json
├── .gitignore
├── LICENSE
├── privacy-policy.md
